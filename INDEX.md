...
| 6.6 | **NEW 2026-08-06 — the right-hand tail. Searches returning 26+ results convert at 2.3%, against 17.8% at 4–25** | ⬜ | Surfaced by plotting s2p per *exact* result count for the Chapter 1 figure; the three-band view (0 / 1–2 / 4+) hid it entirely. 471 searches, **5.2% of all**. Both halves collapse: CTR 25.7% vs 54.9%, click→purchase 9.1% vs 32.4%. The queries concentrated there are `sushi`, `brunch`, `paintball`, `bowling`, `escape room`, `pilates`, `crossfit` — **overwhelmingly the concepts this catalogue stocks nowhere**, i.e. the F1 signature. **Not yet a claim.** The explainer plots it and explicitly declines to explain it; nothing in Chapter 4's sizing credits it. **If it holds, the 52.5% dead-end headline is an understatement** — these 471 are dead ends the ≤2 rule does not catch. Resolve into `FINDINGS.md` before Part C quotes any headline |
