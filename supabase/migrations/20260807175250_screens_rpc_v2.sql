...
ALTER TABLE public.demand_events
  DROP CONSTRAINT demand_events_source_check;

DROP VIEW public.v_demand_by_cell;
...
begin
  ...
  -- This insert reaches the table because the function is SECURITY DEFINER, owned
  -- by postgres, and demand_events is not FORCE RLS — the append policy now
  -- permits notify_me ONLY, so no client can forge a system abstention over REST.
...
ALTER TABLE public.demand_events
  ADD CONSTRAINT demand_events_source_check CHECK (source = ANY (ARRAY['seed'::text, 'live'::text, 'notify_me'::text]));

ALTER TABLE public.search_config
  ADD COLUMN alt_floor numeric NOT NULL;

COMMENT ON COLUMN public.search_config.alt_floor IS 'Similarity floor for the labelled-alternatives block (010-screens SPEC decision 18). An OWNER JUDGEMENT, not a calibration — same standing as high. Above it the top 3 city-stocked services are shown labelled; below it they are suppressed, because GB London''s closest stocked thing to paintball is a manicure at 0.251. Lives here so no component carries the number as a literal.';

CREATE VIEW public.v_demand_by_cell WITH (security_invoker=true) AS SELECT market,
    city,
    concept,
    COALESCE(sum(n) FILTER (WHERE (source = 'seed'::text)), (0)::bigint) AS seed_searches,
    count(*) FILTER (WHERE (source = 'live'::text)) AS live_abstentions,
    count(*) FILTER (WHERE (source = 'notify_me'::text)) AS notify_requests,
    max(occurred_on) AS last_seen
   FROM public.demand_events
  GROUP BY market, city, concept;

COMMENT ON VIEW public.v_demand_by_cell IS 'The acquisition brief (001 SPEC §5), cut PER SOURCE and never summed. seed_searches is the June log and is the ONLY column the abstain state may call "searches" (GB London adrenaline = 222). live_abstentions counts times this system had nothing; notify_requests counts BUTTON PRESSES, not people — there are no sessions or user IDs, so duplicates cannot be deduplicated. A combined total would let a demo click silently join the June log, which is why no such column exists.';

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.v_demand_by_cell TO service_role;

ALTER POLICY demand_events_append ON public.demand_events WITH CHECK (((source = 'notify_me'::text) AND (n = 1)));

-- ---------------------------------------------------------------------------
-- END-STATE ACL, STATED ABSOLUTELY. Hand-written: a schema diff emits DELTAS,
-- and the absence of a `revoke` is exactly the defect this block exists to close
-- (INDEX.md 4.1). Note what the generated half above did and did not do: it
-- re-granted the recreated v_demand_by_cell to service_role and to NOBODY ELSE,
-- because dropping a view drops its grants and the diff had no delta to emit.
--
-- WHY ABSOLUTE AND NOT ADDITIVE. local (supabase CLI) and the remote project
-- carry OPPOSITE default ACLs — local anon=Dxtm, remote anon=arwd — so an
-- additive migration produced a locked-down local database and a wide-open
-- remote one. `revoke all` then grant back lands identically on any target
-- regardless of what it started from.
--
-- THE ONE THAT WOULD HAVE GONE SILENT: search_config was granted in
-- 20260807151103, i.e. AFTER 20260806222319's revoke, so a bare `revoke all`
-- strips it. Nothing would error — search_deals is SECURITY DEFINER and keeps
-- reading it — the staff panel would just quietly lose LOW, HIGH, alt_floor and
-- both error rates. Granted explicitly below.

revoke all on all tables in schema public from anon, authenticated;

-- Public read. The catalogue, the Part A contract and the calibration are the
-- published artifact. Views included: `all tables` covers them, and both were
-- stripped a moment ago.
...
grant select on public.search_config      to anon, authenticated;
...
-- The only write path in the entire schema, and it is now NARROWER than it was.
-- The append policy permits source='notify_me', n=1 and nothing else: an anon
-- REST insert of source='live' FAILS. `live` rows are written only by
-- search_deals, which is SECURITY DEFINER and owned by postgres, so its insert
-- bypasses RLS. Result: a forged "the system had nothing" row is no longer
-- possible over REST, and no demo click can inflate the seeded June baseline.
grant select, insert on public.demand_events to anon, authenticated;

-- service_embeddings and query_embeddings: deliberately NO grant. Combined with
-- RLS-enabled-and-no-policy that is two independent locks. Vectors are read only
-- by the definer RPC, under its own privileges.

-- Functions, same treatment. Postgres grants EXECUTE to PUBLIC on every new
-- function, and anon/authenticated inherit from PUBLIC — so an additive grant
-- here says nothing about what is actually reachable. State it absolutely.
revoke all on all functions in schema public from public, anon, authenticated;
grant execute on function public.fold_query(text)                          to anon, authenticated;
grant execute on function public.search_deals(text, text, text, integer)   to anon, authenticated;
