...
begin
  ...
  if v_band = 'abstain' or v_city_empty then
    insert into public.demand_events (market, city, concept, raw_query, source, n)
    values (p_market, p_city, v_class.concept, q_exact,
            case when v_band = 'abstain' then 'live' else 'city_empty' end, 1);
  end if;
...
COMMENT ON COLUMN public.demand_events.source IS 'FOUR SIGNALS, FOUR MEANINGS, NEVER ONE NUMBER. seed = the June log, the only source the word "searches" may describe. live = the market scored below LOW, so the system had nothing anywhere (an abstention). city_empty = the market scored ABOVE low but this city stocks none of it — a distribution gap, not a semantic one, and a different owner. notify_me = button presses, not people: there are no sessions or user IDs, so duplicates cannot be deduplicated. live and city_empty are written server-side by search_deals (security definer); the RLS append policy permits notify_me only, so neither can be forged over REST.';

ALTER TABLE public.demand_events
  ADD CONSTRAINT demand_events_source_check CHECK (source = ANY (ARRAY['seed'::text, 'live'::text, 'city_empty'::text, 'notify_me'::text]));
...
CREATE VIEW public.v_demand_by_cell WITH (security_invoker=true) AS SELECT market,
    ...
    count(*) FILTER (WHERE (source = 'city_empty'::text)) AS city_empty_events,
...
COMMENT ON VIEW public.v_demand_by_cell IS 'The acquisition brief (001 SPEC §5), cut PER SOURCE and never summed. seed_searches is the June log and is the ONLY column the abstain state may call "searches" (GB London adrenaline = 222). live_abstentions counts times the market had nothing anywhere. city_empty_events counts times the market scored above LOW but THIS CITY stocked none of it — kept separate because it is a distribution gap with a different owner, and folding it into live_abstentions would silently mislabel a number the brief already displays. notify_requests counts BUTTON PRESSES, not people. A combined total would let a demo click join the June log, which is why no such column exists. NOTE: city_empty_events is a staff-side fact only — no user-facing copy may turn it into "available in another city" (010 SPEC CUT; INDEX #12: that copy shipped once and was false for 315 of 321 rows).';
...
-- ---------------------------------------------------------------------------
-- ACL, RE-STATED ABSOLUTELY FOR THE ONE OBJECT THIS MIGRATION DROPS.
-- The diff above did emit the two grants, but the whole INDEX 4.1 lesson is that
-- a dropped-and-recreated object's privileges must not depend on a diff noticing
-- them, nor on the target's default ACLs (local anon=Dxtm, remote anon=arwd).
-- 20260807175250's `alter default privileges ... revoke all` already denies any
-- ambient grant to a newly created view on either target; this states the intended
-- end result outright so the file lands identically wherever it runs.
revoke all on public.v_demand_by_cell from anon, authenticated;
grant select on public.v_demand_by_cell to anon, authenticated;

-- Nothing else in this migration creates or replaces a grantable object:
-- search_deals is CREATE OR REPLACE, which preserves its existing ACL, and
-- demand_events is only altered. The full end-state ACL remains 20260807175250's.
