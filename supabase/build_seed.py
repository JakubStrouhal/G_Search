...
# The alternatives floor, 010-screens SPEC decision 18. Same standing as HIGH: an
# OWNER JUDGEMENT, not a calibration — nothing in the supplied data labels "is
# this a reasonable substitute". It lives here rather than in a Vue template
# because a number in a component is the literal G6 exists to stop. Verified
# against local when it was chosen: it splits the 419 abstain pairs 186 show /
# 233 suppress, and splits them meaningfully — adrenaline suppresses (skydiving
# 0.215, rafting 0.179), `eyelash extensions` GB shows beauty at 0.396.
ALT_FLOOR = 0.30
...
NOTE = ("LOW is calibrated against query_classes.coverage; the rates are BY SEARCH VOLUME at LOW "
        ...
        "ALT_FLOOR is the same standing as HIGH: an owner judgement (010-screens SPEC decision 18), "
        "not a swept value. "
        "Source: docs/analysis/008-threshold-sweep/RESULT.md")
emit("search_config",
     ["low", "high", "alt_floor", "model", "calibrated_on", "n_labelled",
      "false_confident", "false_abstain", "note"],
     [(_sweep["low"], HIGH_JUDGEMENT, ALT_FLOOR, _sweep["model"], CALIBRATED_ON, _sweep["n_pairs"],
