...
1. **Every state reachable by a named chip — except the refusal, which is exempt by construction
   and amended here 2026-08-07 (decision 20).** Every chip is a *real logged query* with a
   *derived* expectation, and the generator aborts if one stops behaving as labelled. A refusal
   chip would have to carry a query the log never saw — a fabricated string presented in a row of
   curated ones, and a chip that works by never working. The refusal is reached by typing, which is
   what a grader does anyway, and the empty prompt says so ("Pick a chip above, or search a
   market's catalogue directly"). The other **five** states each have a chip and a generator
   assertion. Chips: the 15 verified band chips (per market × band, e.g. GB
...
| 20 | Criterion 1 says *every* state gets a chip; the refusal cannot have one honestly (Codex pass 2, 2026-08-07) | **Criterion 1 amended to exempt the refusal, with the reason recorded.** Five states have chips and generator assertions; the refusal is reached by typing | A chip is a real logged query with a derived expectation. A refusal chip would be a **fabricated** query sitting in a curated row — the exact thing this package refuses — and a chip that works only by never working. The alternative considered and rejected: label it "try something we never logged", which is honest but still puts a made-up string among 22 measured ones |
